...
사용자가 중복 이메일로 연락처를 저장하려 할 때 표시되는 오류 메시지는 "이메일은 고유해야 합니다."이며, 이는 @fig-emailerror 에서 볼 수 있습니다.
...
@fig-clicktoload 에서는 클릭 시 연락처의 다음 세트를 페이지에 직접 로드할 수 있는 버튼이 있습니다. 이렇게 하면 현재 연락처가 페이지에서 시각적으로 "컨텍스트"를 유지할 수 있으며, 여전히 정상적인 페이지된 사용자 인터페이스처럼 진행할 수 있습니다.
