...
이는 초기 웹 개발 시대와 대조적입니다. 그때는 _표현 상태 전송(REST)_과 _하이퍼미디어를 애플리케이션 상태의 엔진으로(HATEOAS)_와 같은 개념이 자주 논의되고 정제되었으며, 웹 개발자들 사이에서 활발히 토론되었습니다.
...
앵커는 오늘날 우리가 웹을 탐색하는 주요 메커니즘을 제공합니다. 문서에서 문서로, 또는 자원에서 자원으로 탐색하기 위해 링크를 선택합니다. @fig-get-in-action 은 앵커 태그/하이퍼링크와의 사용자 상호작용이 어떤 모습인지 시각적으로 보여줍니다.
...
이는 사실이며, 우리는 이후 장에서 양식에 대한 구체적으로 깊이 들어가겠지만, 지금은 이 간단한 예가 하이퍼미디어 내에서 시스템 상태를 업데이트하는 핵심 메커니즘을 보여주기에 충분합니다. @fig-post-in-action 은 그 상호작용의 다이어그램입니다.
