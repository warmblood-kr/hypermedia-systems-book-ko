...
오늘날 이상하게도 "REST"라는 용어는 HTML과 하이퍼미디어보다는 JSON 데이터 API와 주로 연관되어 있습니다. 이는 매우 웃긴 일입니다. 왜냐하면 대다수의 JSON 데이터 API가 원래의 의미에서 RESTful이 아니며, 사실상 RESTful이 될 수 _없기_ 때문입니다. 그 이유는 그들이 자연적인 하이퍼미디어 형식을 사용하지 않기 때문입니다.
...
#index[HTML][spec]
HTML에 대한 가장 권위있는 자료는 HTML 스펙입니다. 현재 사양은
#link("https://html.spec.whatwg.org/multipage").#footnote[단일 페이지 버전은 대부분의 컴퓨터에서 너무 느리게 로드되고 렌더링됩니다.
